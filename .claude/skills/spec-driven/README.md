...
- **Atomic git commits** — One task = one commit, using [**ticket-prefix** format](../../rules/commit-pattern.mdc) (`[<ticket-code>]: …`):

```
[ORG-100]: add email validation to login form

[ORG-101]: extract token refresh logic into service

[ORG-102]: prevent negative quantity on item decrement
```
...
Agent: Quick Task: Fix token refresh expired check
       ...
       ✅ Done. Committed: [JIRA-100]: add expiry check to token refresh
```
...
Agent: Implementing T1: Create auth service interface
       ...
       ✅ Committed: [JIRA-200]: create auth service interface
...
Agent: Quick Task: Persist dark mode preference
       ...
       ✅ Committed: [JIRA-201]: persist dark mode preference to localStorage
```
...
A: Each task produces exactly one commit using the **[ticket-code]: description** format in `commit-pattern.mdc`. That keeps history readable, bisect-friendly, and easy to roll back.
